...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (
        ...
        isset($_POST["etat_rec"]) && isset($_POST["type"]) && isset($_POST["date_overture"]) && isset($_POST["date_fermuture"])
    ) {
        if (
            !empty($_POST["id_recl"]) && !empty($_POST["id_user"]) && !empty($_POST["description"]) && 
            !empty($_POST["etat_rec"]) && !empty($_POST["type"]) && !empty($_POST["date_overture"]) && !empty($_POST["date_fermuture"])
        ) {
  ...
  <body>
    ...
    <section>
        ...
        <form action="" method="post" class="container mt-5">
          ...
          <div class="mb-3">
            <label for="date_fermuture" class="form-label">Date Fermeture</label>
            <input type="date" class="form-control" id="date_fermuture" name="date_fermuture" placeholder="Select Date Fermeture">
          </div>
