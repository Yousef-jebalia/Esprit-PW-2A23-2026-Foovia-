document.addEventListener('DOMContentLoaded', () => {
  ...
  const deliveryPhoneInput = document.querySelector('[data-delivery-phone]');
  ...
  if (deliveryCashButton) {
      deliveryCashButton.addEventListener('click', () => {
      if (!selectedHub) {
        if (deliveryFeedbackNode) {
      ...
      if (deliveryFeedbackNode) {
        deliveryFeedbackNode.textContent = 'Enter your phone number for cash delivery updates.';
      }
      return;
      }
      ...
      renderCart();
      if (deliveryFeedbackNode) {
        deliveryFeedbackNode.textContent = `Cash order confirmed. Estimated delivery time: ${formatMinutes(currentEtaMinutes ?? estimateDeliveryMinutes(selectedHub, deliveryDestination))}. Delivery fee: ${formatPrice(currentDeliveryFee)} TND.`;
      }
      window.setTimeout(() => {
        if (deliveryPlanner) {
          deliveryPlanner.hidden = true;
